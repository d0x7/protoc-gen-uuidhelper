...
// Versions:
// - protoc-gen-uuidhelper-kotlin v0.0.2-dev.4+dirty
// - protoc v5.29.3
// source: test.proto
...
fun PlayerKt.Dsl.SetGameUUIDs(value: Collection<UUID>) {
	...
	for (i in 0 until value.size) {
		list.add(uuidToByteString(value.elementAt(i)))
	}
	gameUuids.clear()
	gameUuids.addAll(list)
}

fun PlayerKt.Dsl.AddGameUUIDs(value: UUID) {
	gameUuids.add(uuidToByteString(value))
}

fun Test.Player.GameUUIDs(): List<UUID> {
	val list = this.gameUuidsList
	val result = mutableListOf<UUID>()
	for (i in 0 until list.size) {
		result.add(byteStringToUUID(list[i]))
	}
